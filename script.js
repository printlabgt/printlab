...
$$('.btn-detail').forEach(btn => {
  btn.addEventListener('click', ()=>{
    ...
    mTags.innerHTML    = (btn.dataset.tags || '')
                      .split(' ')
                      .map(t=>`<span>${t}</span>`)
                      .join(' ');
    ...
    // 🔹 Nuevo: solo tocar mModel si existe
    if (mModel) {
...
// También abrir el visor al hacer clic en la imagen de la tarjeta (MDF)
$$('.grid[data-grid="mdf"] .card img').forEach(img => {
  img.addEventListener('click', () => {
    const card = img.closest('.card');
    const btn  = card?.querySelector('.btn-detail');
    if (btn) btn.click();     // reutiliza la misma lógica del botón
  });
});

// También abrir el visor al hacer clic en la imagen de la tarjeta
$$('.grid[data-grid="mdf"] .card img').forEach(img => {
  img.addEventListener('click', () => {
    const card = img.closest('.card');
    const btn  = card?.querySelector('.btn-detail');
